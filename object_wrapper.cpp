...
bool ObjectWrapper::addParameters() {
  ...
  //DOES IT GO OUT OF SCOPE???? (solution; i think it does not og out of scope, seems smart enough)
   std::function<void(const std::string)> lambda_fun=[this](std::string s){this->initpop(s); std::cout << "inside addParam " << s << std::endl;};
...
void ObjectWrapper::initpop(std::string pop){
   ...
   std::cout << initnewSpecies.mass_units << std::endl;
   ...
   auto newSpecies=&getObjectWrapper().particleSpecies.at(species_i);
