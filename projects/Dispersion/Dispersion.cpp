...
namespace projects {
   ...
   void Dispersion::addParameters() {
      ...
      RP::add("Dispersion.B0", "Guide magnetic field strength (T)", this->B0);
      RP::add("Dispersion.magXPertAbsAmp", "Absolute amplitude of the magnetic perturbation along x (T)", this->magXPertAbsAmp);
      RP::add("Dispersion.magYPertAbsAmp", "Absolute amplitude of the magnetic perturbation along y (T)", this->magYPertAbsAmp);
      RP::add("Dispersion.magZPertAbsAmp", "Absolute amplitude of the magnetic perturbation along z (T)", this->magZPertAbsAmp);
      RP::add("Dispersion.maxwCutoff", "Cutoff for the maxwellian distribution", this->maxwCutoff);
      RP::add("Dispersion.angleXY", "Orientation of the guide magnetic field with respect to the x-axis in x-y plane (rad)", this->angleXY);
      RP::add("Dispersion.angleXZ", "Orientation of the guide magnetic field with respect to the x-axis in x-z plane (rad)", this->angleXZ);

      // Per-population parameters
      for(uint i=0; i< getObjectWrapper().particleSpecies.size(); i++) {

        DispersionSpeciesParameters newsP;
        ...
        RP::add(pop + "_Dispersion.rho", "Number density (m^-3)", sP->DENSITY);
        RP::add(pop + "_Dispersion.Temperature", "Temperature (K)", sP->TEMPERATURE);
        RP::add(pop + "_Dispersion.densityPertRelAmp", "Relative amplitude of the density perturbation", sP->densityPertRelAmp);
        RP::add(pop + "_Dispersion.velocityPertAbsAmp", "Absolute amplitude of the velocity perturbation", sP->velocityPertAbsAmp);
      }
   }
